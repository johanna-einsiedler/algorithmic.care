...
def read_raw_signal(file):
    ...
    df.loc[(30<df['time'])*(df['time']<36),'interval'] = 'muscle_contraction_1'
    df.loc[(36<df['time'])*(df['time']<46),'interval'] = 'rest_1'
    df.loc[(46<df['time'])*(df['time']<52),'interval'] = 'muscle_contraction_2'
    df.loc[(52<df['time'])*(df['time']<62),'interval'] = 'rest_2'
    df.loc[(62<df['time'])*(df['time']<68),'interval'] = 'muscle_contraction_3'
...
if __name__ == "__main__":
    ...
    if os.path.isfile(path):
        ...
        with open('../data_pre_processed/'+(df['date'].astype(str)+'_'+df['am_pm']).unique()[0]+'.pickle', 'wb') as handle:
