...
<div class="grid grid-cols-3">
 ...
 <div>
  ...
  <h2> Data Visualisation | N=1 | Amanda </h2>
...
const surveyData = FileAttachment("data/data_amanda.csv").csv({typed: true})//.then((D) => D.map(coerceRow));
const EMGData = FileAttachment("data/emg_amanda.csv").csv({typed: true})//.then((D) => D.map(coerceRow));
const annotation = FileAttachment("data/annotation_AB.csv").csv({typed: true})//.then((D) => D.map(coerceRow));
...
let lastEntry = { [lastKey]: transformedData[lastKey] };
...
function BubblePlot(width) {
  ...
  marks: [
    ...
    // grey discs
   Plot.geo([0.05, 0.05+5000/300000, 0.05+10000/300000,0.05+15000/300000,0.05+20000/300000,0.18, 0.2, 0.22,0.24,0.29, 0.31, 0.36,0.4,0.42], {
 ...
 */
  ...
  // points for CIS scale
    Plot.dot(transformedData.filter(item => Array('cis_subjective_fatigue','cis_concentration', 'hooper_stress_level','cis_physical_activity').includes(item.name)),
     ...
     tip:true,
          title:  (d) =>
          `${monthNames[new Date(d.date).getMonth()]} ${new Date(d.date).getDate()} \nPercent of Mean: ${Math.round(d.value/meansFatigue[d.name]*100)}`
   ...
   Plot.text(transformedData, {
    ...
    Plot.lineY(EMGDataNew.filter(item => Array('signal_emg','emg_envelope','signal_mmg').includes(item.name)),
     {x: ({date}) => longitude(date), 
      y: ({ position, value }) => 90-Number(position) - value/15 ,
    curve: "cardinal-open",
      stroke: ({name}) => colorScale[name]
      }),
