...
<div class="hero">
<h1> N=2 </h1>
<h2> Making Sense of our Data </h2>

<div class="grid grid-cols-2">
<div>
<h3>Subjective Experience </h3>

Everyday we answered a set of questions about our subjective levels of productivity, concentration, stress and fatigue, scored on a scale from 1 -7. We further tried to predict how fatigued we would feel on the next day.

</div>
<div>
<img src="images/fatigue_pred_legend.png" alt="drawing" width="300"/>
...
<div class="grid grid-cols-2">
<div>
<h3>Perceived physical status </h3>
Everyday we self-assessed how our muscles were feeling and tried to predict how sore and tired we expected our muscles to be on the next day.
</div>
<div>
    <img src="images/muscle_legend.png" alt="drawing" width="300"/>
    </div>
  </div>
<div class="grid grid-cols-2">
<div>
<h3>EMG & MMG Traces </h3>
Every morning and evening we took measurements of our left, outer side of the thigh muscles - following a protocol. The outer ring shows the electromyogram and mechanomyogram during a 6s muscle contraction.
</div>
<div></div>
</div>
...
<div class="grid grid-cols-3">
 ...
 <img src="images/sleep_steps_legend.png" alt="drawing" width="400"/>
 </div>
 <div>
  <img src="images/physical_activity_legend.png" alt="drawing" width="200"/>
  </div>
  </div>
  <h2> Data Visualisation | N=2 | Johanna </h2>
</div>
...
const annotation = FileAttachment('data/annotation_JE.csv').csv({ typed: true }) //.then((D) => D.map(coerceRow));
```

```js
// pivot data longer
let data = aq
  .from(surveyData) // create an arquero table from our data
  .fold(aq.not('date'), { as: ['name', 'value'] }) // unpivot/fold the data by year, create columns Month and Unemployment
  .objects()

let EMGDataNew = aq
  .from(EMGData) // create an arquero table from our data
  .fold(aq.not('date'), { as: ['name', 'value'] }) // unpivot/fold the data by year, create columns Month and Unemployment
  .objects()
...
let newData = data.filter(item =>
  Array(
    ...
    'alcohol',
    'fatigue_prediction',
    'muscles_tired_prediction',
    'muscles_sore_prediction',
    'step_count',
    '1th_col_with_one',
    '2th_col_with_one',
    '3th_col_with_one'
...
const dictionaryImages = {
  walking:
    'https://raw.githubusercontent.com/johanna-einsiedler/algorithmic.care/main/website/docs/data/images/walking.png',
  cycling:
    'https://raw.githubusercontent.com/johanna-einsiedler/algorithmic.care/main/website/docs/data/images/cycling.png',
  ball_sports:
    'https://raw.githubusercontent.com/johanna-einsiedler/algorithmic.care/main/website/docs/data/images/ball_sports.png',
  manual_work:
    'https://raw.githubusercontent.com/johanna-einsiedler/algorithmic.care/main/website/docs/data/images/manual_work.png',
  running:
    'https://raw.githubusercontent.com/johanna-einsiedler/algorithmic.care/main/website/docs/data/images/running.png',
  weight_lifting:
    'https://raw.githubusercontent.com/johanna-einsiedler/algorithmic.care/main/website/docs/data/images/weight_lifting.png',
...
let longitude = d3
  .scalePoint(new Set(Plot.valueof(EMGDataNew, 'date')), [180, -160])
  .padding(0.5)
  .align(1)
...
const niceNames = {
  ...
  hooper_muscles_sore: 'My muscles felt sore',
  extra_muscles_tired: 'My muscles felt tired',
...
let keys = Object.keys(longitude.domain())

// Get the last key
let lastKey = keys[keys.length - 1]

// Get the last entry (key and value)
let lastEntry = { [lastKey]: longitude.domain()[lastKey] }

function BubblePlot(width) {
  return Plot.plot({
    r: { range: [0, 26] },
  width:  width*1.2,
  marginLeft: -width/5,
  marginTop: -width/10,
    ...
    marks: [
      ...
      Plot.lineY(transformedData, {
        x: ({ date }) => longitude(date) - 36,
        y: ({ date }) => (longitude(date) <= -100 ? 90 - 0.5 : 90),
        fill: '#fff9f1',
        type: 'cardinal-open'
      }),
 ...
 */
      ...
      Plot.link(
        transformedData.filter(item => Array('step_count').includes(item.name)),
        {
          x1: ({ date }) => longitude(date),
          x2: ({ date }) => longitude(date),
      ...
      Plot.dot(
        ...
        ),
        {
          ...
          y: ({ position }) => 90 - Number(position),
          r: ({ value, name }) =>
            null != value ? (1 - value / meansFatigue[name]) ** 2 : null,
          fill: ({ name }) => colorScale[name],
          symbol: 'triangle',
          rotate: ({ value, date, name }) =>
            value / meansFatigue[name] < 1
              ? 180 - Number(longitude(date)) + 180
              : 180 - Number(longitude(date)),
          opacity: 0.6,
          tip: true,
          title: d =>
            `${monthNames[new Date(d.date).getMonth()]} ${new Date(
              d.date
            ).getDate()} \nPercent of Mean: ${Math.round(
              (d.value / meansFatigue[d.name]) * 100
            )}`
      ...
      Plot.dot(
        transformedData.filter(item =>
          Array('fatigue_prediction').includes(item.name)
        ),
        {
          x: ({ date }) => longitude(date), //longitude(date),
          y: ({ position }) => 90 - Number(position),
          r: ({ value, name }) =>
            null != value
              ? (1 - value / meansFatigue['cis_subjective_fatigue']) ** 2
              : null,
          stroke: ({ name }) => colorScale[name],
          symbol: 'triangle',
          rotate: ({ value, date, name }) =>
            value / meansFatigue['cis_subjective_fatigue'] < 1
              ? 180 - Number(longitude(date)) + 180
              : 180 - Number(longitude(date))
        }
      ),
      ...
      Plot.text(transformedData, {
        ...
        angel: 90,
        fontFamily:
          "url(/Users/htr365/Documents/Side_Projects/09_founding_lab/amanda_johanna/quantified_self.nosync/website/docs/fonts/OpenSauceOne-Black.ttf')"
      }),

      // recatangles for muscle data
      Plot.dot(
        transformedData.filter(item =>
          Array('hooper_muscles_sore', 'extra_muscles_tired').includes(
            item.name
          )
        ),
        {
          ...
          title: d =>
            `${monthNames[new Date(d.date).getMonth()]} ${new Date(
              d.date
            ).getDate()} \n ${niceNames[d.name]}: ${likertScale[d.value]}`
          //stroke: "white",
        }
      ),
      ...
      Plot.dot(
        transformedData.filter(item =>
          Array('muscles_sore_prediction', 'muscles_tired_prediction').includes(
            item.name
          )
        ),
        {
      ...
      Plot.dot(
        transformedData.filter(item =>
          Array('hooper_sleep_quality').includes(item.name)
        ),
        {
          x: ({ date }) => longitude(date), //longitude(date),
          y: ({ position }) => 90 - Number(position),
      ...
      Plot.text(
        transformedData.filter(item =>
          Array('sleep_hours').includes(item.name)
        ),
        {
          x: ({ date }) => longitude(date), //longitude(date),
          y: ({ position }) => 90 - Number(position),
          ...
          fill: ({ date }) =>
            transformedData
              .filter(item => Array('hooper_sleep_quality').includes(item.name))
              .filter(item => Array(date).includes(item.date))[0].value <= 4
              ? 'black'
              : 'white',
          title: d =>
            `${monthNames[new Date(d.date).getMonth()]} ${new Date(
              d.date
            ).getDate()} Sleep Duration: ${d.value} hours \nSleep Quality was ${
              likertSleep[
                transformedData
                  .filter(item =>
                    Array('hooper_sleep_quality').includes(item.name)
                  )
                  .filter(item => Array(d.date).includes(item.date))[0].value
              ]
            }`,
          tip: true
        }
      ...
      Plot.image(
        transformedData.filter(item =>
          Array('coffee_count').includes(item.name)
        ),
        {
          x: ({ date }) => longitude(date),
      ...
      // alcohol
      Plot.image(
        transformedData.filter(item => Array('alcohol').includes(item.name)),
        {
          x: ({ date }) => longitude(date),
          y: ({ position }) => 90 - position,
      ...
      // area EMG stats
      Plot.lineY(
        EMGDataNew.filter(item =>
          Array('signal_emg', 'emg_envelope', 'signal_mmg').includes(item.name)
        ),
        {
          x: ({ date }) => longitude(date),
